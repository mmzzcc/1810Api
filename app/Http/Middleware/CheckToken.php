...
{   
    ...
    {   
        ...
        if ($check['status']=='1000') {
          ...
          #接收返回的数据，对返回的数据进行加密
          $api_response=[];
          $api_response['data']=$this->_AesEncrypt($response->original);
    ...
    private function _AesEncrypt($data)
    {
      ...
      $enc_data=base64_encode(openssl_encrypt($json_str,'AES-256-CBC',$key, OPENSSL_RAW_DATA,$iv));
      return $enc_data;
    }
    /**
     * 解密客户端的数据
    ...
    private function _AesDecrypt($request)
    {
      ...
      $key=env('ENCRYPT_KEY');
      $iv=env('ENCRYPT_IV');
      $dec_data=openssl_decrypt(base64_decode($request->post('data')), 'AES-256-CBC', $key,OPENSSL_RAW_DATA,$iv);
